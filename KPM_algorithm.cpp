...
#define position 0//KPM进行比较时本程序从0开始进行比较，之后可以进行修改。
#define none 0//未输入
//使用KMP算法进行处理
typedef struct str
{
...
Status get_next(String* Target, int* next)
{
	...
	int maxsize = 0;//maxsize为可以满足条件的最大长度值。
	while (i < Target->CurLen)
	{
		if (maxsize == 0 || Target->base[i] == Target->base[maxsize])
		{
			i++;
			maxsize++;
			next[i] = maxsize;
		}
		else maxsize = next[maxsize];//当不满足上述条件的时候，直接进行处理，让maxsize取到next【maxsize】。
	}
	return OK;
}//定义取用下一位的操作
Status Index_KPM(String* Station, String* Target, int pos,int* result)
//S为目标串，T为模式串，pos为起始比较位置，result为返回的值参数。
{
	...
	int i = pos;
	int maxsize = 1;
	while (i <= Station->CurLen && maxsize <= Target->CurLen)
	{
		if (maxsize == 0 || Station->base[i] == Target->base[maxsize])
		{
			++maxsize;
			++i;
		...
		{
			maxsize = next[maxsize];//如果匹配失败，直接进入next[maxsize]位置进行匹配与判断。这个时候i是不动的。
		}
	}
	free(next);
	if (maxsize > Target->CurLen)
	{
		result[statu] = OK;
	...
	{
		result[statu] = FALSE;
		return FALSE;//当maxsize无法大于T串长度时，认为其没有办法完成全部T串的比较。直接失效。
	}
}//执行KPM算法对于字符串进行分析拿到首位地址
Status IsError01(int argc,char** argv)
{
	if (!argv) return ERROR;
	if (argc ==correct) return FALSE;
	else return TRUE;
}//判断是否命令行参数错误。
...
int final_Index_Package(int argc, char** argv,int *result)
{
	if (!result) return ERROR;
	if (!argv) return ERROR;
	if (IsError01(argc,argv))
	{
		printf("ERROR_01");
		return ERROR_01;
	}//判断是否命令行参数错误.
	...
	Strassign(Target,argv[1]);//将输入的命令行参数存放进目标串中
	Strassign(Target,argv[2]);//将输入的命令行参数存放进模式串中

	Index_KPM(Station, Target,position,result);//KPM算法
	free(Station);
	free(Target);
	return 0;
}//进行KMP寻址之前的调用与初始化函数
